...
export class TeachingExperienceStepComponent implements OnInit {
  ...
  constructor(
    ...
    this.formGroup = this._formBuilder.group({
      experience: ['', [Validators.required, Validators.minLength(50), Validators.maxLength(300)]],
    });
  }

  ngOnInit() {
    let teacher = history.state.teacher;
    if ('experience' in history.state.teacher) {
      this.initFormWithCachedData(teacher);
    }
  }

  private initFormWithCachedData(teacher) {
    this.formGroup.controls['experience'].setValue(teacher.experience);
  }
}

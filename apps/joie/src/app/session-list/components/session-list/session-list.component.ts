...
export class SessionListComponent implements OnInit {
  ...
  getItems() {
    this.sessionsService.getSessionsData('sessions', this.pageCount, this.queryFn).subscribe(
      (response: any) => {
        if (!response.length) {
          return false;
        }
  ...
  nextPage() {
    ...
    this.sessionsService.getSessionsNext('sessions', this.pageCount, this.lastInResponse, this.queryFn).subscribe(
      (response: any) => {
        if (!response.length) {
          this.disable_next = true;
          return;
  ...
  prevPage() {
    ...
    this.sessionsService
      .getSessionsPrev('sessions', this.pageCount, this.get_prev_startAt(), this.firstInResponse, this.queryFn)
      .subscribe(
        (response: any) => {
          this.firstInResponse = response[0].payload.doc;
          this.lastInResponse = response[response.length - 1].payload.doc;
